...
{	Trans *T;
	...
	T = T->nxt	= settr(167,0,3,0,0,"IF", 0, 2, 0);
	    T->nxt	= settr(167,0,6,0,0,"IF", 0, 2, 0);
	trans[2][3]	= settr(162,0,4,26,0,"((doors_status.higher==1))", 1, 2, 0);
	trans[2][4]	= settr(163,0,5,27,27,"change_doors_pos!1", 1, 7, 0);
	trans[2][5]	= settr(164,0,15,28,28,"doors_pos_changed?1", 1, 508, 0);
	trans[2][9]	= settr(168,0,15,1,0,".(goto)", 0, 2, 0);
	trans[2][6]	= settr(165,0,7,2,0,"else", 0, 2, 0);
	trans[2][7]	= settr(166,0,15,1,0,"(1)", 0, 2, 0);
	T = trans[2][15] = settr(174,0,0,0,0,"IF", 0, 2, 0);
	T = T->nxt	= settr(174,0,10,0,0,"IF", 0, 2, 0);
	    T->nxt	= settr(174,0,13,0,0,"IF", 0, 2, 0);
	trans[2][10]	= settr(169,0,11,29,0,"((slide_status.higher==1))", 1, 2, 0);
	trans[2][11]	= settr(170,0,12,30,30,"change_slide_pos!1", 1, 9, 0);
	trans[2][12]	= settr(171,0,30,31,31,"slide_pos_changed?1", 1, 510, 0);
	trans[2][16]	= settr(175,0,30,1,0,".(goto)", 0, 2, 0);
	trans[2][13]	= settr(172,0,14,2,0,"else", 0, 2, 0);
	trans[2][14]	= settr(173,0,30,1,0,"(1)", 0, 2, 0);
	T = trans[2][30] = settr(189,0,0,0,0,"IF", 0, 2, 0);
	T = T->nxt	= settr(189,0,17,0,0,"IF", 0, 2, 0);
	    T->nxt	= settr(189,0,28,0,0,"IF", 0, 2, 0);
	trans[2][17]	= settr(176,0,26,32,0,"((doors_status.lower==2))", 1, 2, 0);
	T = trans[2][26] = settr(185,0,0,0,0,"IF", 0, 2, 0);
	T = T->nxt	= settr(185,0,18,0,0,"IF", 0, 2, 0);
	    T->nxt	= settr(185,0,23,0,0,"IF", 0, 2, 0);
	trans[2][18]	= settr(177,0,19,33,0,"(((lock_water_level!=2)&&(slide_status.lower==2)))", 1, 2, 0);
	trans[2][19]	= settr(178,0,20,34,34,"change_slide_pos!2", 1, 9, 0);
	trans[2][20]	= settr(179,0,21,35,35,"slide_pos_changed?1", 1, 510, 0);
	trans[2][21]	= settr(180,0,22,36,36,"change_doors_pos!2", 1, 7, 0);
	trans[2][22]	= settr(181,0,32,37,37,"doors_pos_changed?1", 1, 508, 0);
	trans[2][27]	= settr(186,0,32,1,0,".(goto)", 0, 2, 0);
	trans[2][23]	= settr(182,0,24,38,0,"((lock_water_level==2))", 1, 2, 0);
	trans[2][24]	= settr(183,0,25,39,39,"change_doors_pos!2", 1, 7, 0);
	trans[2][25]	= settr(184,0,32,40,40,"doors_pos_changed?1", 1, 508, 0);
	trans[2][31]	= settr(190,0,32,1,0,".(goto)", 0, 2, 0);
	...
	T = T->nxt	= settr(200,0,36,0,0,"IF", 0, 2, 0);
	    T->nxt	= settr(200,0,39,0,0,"IF", 0, 2, 0);
	trans[2][36]	= settr(195,0,37,46,0,"((doors_status.lower==1))", 1, 2, 0);
	trans[2][37]	= settr(196,0,38,47,47,"change_doors_pos!2", 1, 7, 0);
	trans[2][38]	= settr(197,0,48,48,48,"doors_pos_changed?1", 1, 508, 0);
	trans[2][42]	= settr(201,0,48,1,0,".(goto)", 0, 2, 0);
	trans[2][39]	= settr(198,0,40,2,0,"else", 0, 2, 0);
	trans[2][40]	= settr(199,0,48,1,0,"(1)", 0, 2, 0);
	T = trans[2][48] = settr(207,0,0,0,0,"IF", 0, 2, 0);
	T = T->nxt	= settr(207,0,43,0,0,"IF", 0, 2, 0);
	    T->nxt	= settr(207,0,46,0,0,"IF", 0, 2, 0);
	trans[2][43]	= settr(202,0,44,49,0,"((slide_status.lower==1))", 1, 2, 0);
	trans[2][44]	= settr(203,0,45,50,50,"change_slide_pos!2", 1, 9, 0);
	trans[2][45]	= settr(204,0,63,51,51,"slide_pos_changed?1", 1, 510, 0);
	trans[2][49]	= settr(208,0,63,1,0,".(goto)", 0, 2, 0);
	trans[2][46]	= settr(205,0,47,2,0,"else", 0, 2, 0);
	trans[2][47]	= settr(206,0,63,1,0,"(1)", 0, 2, 0);
	T = trans[2][63] = settr(222,0,0,0,0,"IF", 0, 2, 0);
	T = T->nxt	= settr(222,0,50,0,0,"IF", 0, 2, 0);
	    T->nxt	= settr(222,0,61,0,0,"IF", 0, 2, 0);
	trans[2][50]	= settr(209,0,59,52,0,"((doors_status.higher==2))", 1, 2, 0);
	T = trans[2][59] = settr(218,0,0,0,0,"IF", 0, 2, 0);
	T = T->nxt	= settr(218,0,51,0,0,"IF", 0, 2, 0);
	    T->nxt	= settr(218,0,56,0,0,"IF", 0, 2, 0);
	trans[2][51]	= settr(210,0,52,53,0,"(((lock_water_level!=1)&&(slide_status.higher==2)))", 1, 2, 0);
	trans[2][52]	= settr(211,0,53,54,54,"change_slide_pos!1", 1, 9, 0);
	trans[2][53]	= settr(212,0,54,55,55,"slide_pos_changed?1", 1, 510, 0);
	trans[2][54]	= settr(213,0,55,56,56,"change_doors_pos!1", 1, 7, 0);
	trans[2][55]	= settr(214,0,65,57,57,"doors_pos_changed?1", 1, 508, 0);
	trans[2][60]	= settr(219,0,65,1,0,".(goto)", 0, 2, 0);
	trans[2][56]	= settr(215,0,57,58,0,"((lock_water_level==1))", 1, 2, 0);
	trans[2][57]	= settr(216,0,58,59,59,"change_doors_pos!1", 1, 7, 0);
	trans[2][58]	= settr(217,0,65,60,60,"doors_pos_changed?1", 1, 508, 0);
	trans[2][64]	= settr(223,0,65,1,0,".(goto)", 0, 2, 0);
