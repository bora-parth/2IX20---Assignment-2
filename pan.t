...
{	Trans *T;
	...
	T = trans[ 4][37] = settr(272,2,0,0,0,"ATOMIC", 1, 2, 0);
	T->nxt	= settr(272,2,1,0,0,"ATOMIC", 1, 2, 0);
	trans[4][1]	= settr(236,2,2,3,3,"(run main_control())", 1, 2, 0);
	trans[4][2]	= settr(237,2,14,4,4,"proc = 0", 1, 2, 0);
	trans[4][15]	= settr(250,2,14,1,0,".(goto)", 1, 2, 0);
	T = trans[4][14] = settr(249,2,0,0,0,"DO", 1, 2, 0);
	T = T->nxt	= settr(249,2,3,0,0,"DO", 1, 2, 0);
	    T->nxt	= settr(249,2,12,0,0,"DO", 1, 2, 0);
	trans[4][3]	= settr(238,2,10,5,5,"((proc<1))", 1, 2, 0); /* m: 4 -> 10,0 */
	reached4[4] = 1;
	...
	trans[4][10]	= settr(245,2,11,6,6,"(run lock(proc))", 1, 2, 0);
	trans[4][11]	= settr(246,2,14,7,7,"proc = (proc+1)", 1, 2, 0);
	trans[4][12]	= settr(247,2,25,8,8,"((proc==1))", 1, 2, 0); /* m: 17 -> 25,0 */
	reached4[17] = 1;
	trans[4][13]	= settr(248,2,17,1,0,"goto :b8", 1, 2, 0); /* m: 17 -> 0,25 */
	reached4[17] = 1;
	trans[4][16]	= settr(251,2,17,1,0,"break", 1, 2, 0);
	trans[4][17]	= settr(252,2,25,9,9,"proc = 0", 1, 2, 0);
	trans[4][26]	= settr(261,2,25,1,0,".(goto)", 1, 2, 0);
	T = trans[4][25] = settr(260,2,0,0,0,"DO", 1, 2, 0);
	T = T->nxt	= settr(260,2,18,0,0,"DO", 1, 2, 0);
	    T->nxt	= settr(260,2,23,0,0,"DO", 1, 2, 0);
	trans[4][18]	= settr(253,2,21,10,10,"((proc<1))", 1, 2, 0); /* m: 19 -> 21,0 */
	reached4[19] = 1;
	trans[4][19]	= settr(0,0,0,0,0,"ship_status[proc] = 3",0,0,0);
	trans[4][20]	= settr(0,0,0,0,0,"ship_pos[proc] = 0",0,0,0);
	trans[4][21]	= settr(256,2,22,11,11,"(run ship(proc))", 1, 2, 0);
	trans[4][22]	= settr(257,2,25,12,12,"proc = (proc+1)", 1, 2, 0);
	trans[4][23]	= settr(258,2,34,13,13,"((proc==1))", 1, 2, 0); /* m: 28 -> 34,0 */
	reached4[28] = 1;
	trans[4][24]	= settr(259,2,28,1,0,"goto :b9", 1, 2, 0); /* m: 28 -> 0,34 */
	reached4[28] = 1;
	trans[4][27]	= settr(262,2,28,1,0,"break", 1, 2, 0);
	trans[4][28]	= settr(263,2,34,14,14,"proc = 0", 1, 2, 0);
	trans[4][35]	= settr(270,2,34,1,0,".(goto)", 1, 2, 0);
	T = trans[4][34] = settr(269,2,0,0,0,"DO", 1, 2, 0);
	T = T->nxt	= settr(269,2,29,0,0,"DO", 1, 2, 0);
	    T->nxt	= settr(269,2,32,0,0,"DO", 1, 2, 0);
	trans[4][29]	= settr(264,2,34,15,15,"((proc<1))", 1, 2, 0); /* m: 30 -> 34,0 */
	reached4[30] = 1;
	...
	trans[4][32]	= settr(267,2,33,2,0,"else", 1, 2, 0);
	trans[4][33]	= settr(268,2,36,1,0,"goto :b10", 1, 2, 0);
	trans[4][36]	= settr(271,0,38,1,0,"break", 1, 2, 0);
	trans[4][38]	= settr(273,0,0,16,16,"-end-", 0, 3500, 0);

	/* proctype 3: monitor */

	trans[3] = (Trans **) emalloc(3*sizeof(Trans *));

	trans[3][1]	= settr(234,0,2,17,0,"assert(1)", 0, 2, 0);
	trans[3][2]	= settr(235,0,0,18,18,"-end-", 0, 3500, 0);

	/* proctype 2: main_control */

	trans[2] = (Trans **) emalloc(75*sizeof(Trans *));

	trans[2][1]	= settr(160,0,71,19,19,"num = 0", 0, 2, 0);
	trans[2][72]	= settr(231,0,71,1,0,".(goto)", 0, 2, 0);
	T = trans[2][71] = settr(230,0,0,0,0,"DO", 0, 2, 0);
	T = T->nxt	= settr(230,0,2,0,0,"DO", 0, 2, 0);
	    T->nxt	= settr(230,0,69,0,0,"DO", 0, 2, 0);
	trans[2][2]	= settr(161,0,65,20,0,"((num<1))", 0, 2, 0);
	trans[2][66]	= settr(225,0,65,1,0,".(goto)", 0, 2, 0);
	T = trans[2][65] = settr(224,0,0,0,0,"DO", 0, 2, 0);
	T = T->nxt	= settr(224,0,3,0,0,"DO", 0, 2, 0);
	    T->nxt	= settr(224,0,34,0,0,"DO", 0, 2, 0);
	trans[2][3]	= settr(162,0,9,21,21,"request_low[num]?1", 1, 503, 0);
	T = trans[2][9] = settr(168,0,0,0,0,"IF", 0, 2, 0);
	T = T->nxt	= settr(168,0,4,0,0,"IF", 0, 2, 0);
	    T->nxt	= settr(168,0,7,0,0,"IF", 0, 2, 0);
	trans[2][4]	= settr(163,0,5,22,0,"((doors_status[num].higher==1))", 1, 2, 0);
	trans[2][5]	= settr(164,0,6,23,23,"change_doors_pos[num]!1", 1, 7, 0);
	trans[2][6]	= settr(165,0,16,24,24,"doors_pos_changed[num]?1", 1, 508, 0);
	...
	trans[2][11]	= settr(170,0,12,25,0,"((slide_status[num].higher==1))", 1, 2, 0);
	trans[2][12]	= settr(171,0,13,26,26,"change_slide_pos[num]!1", 1, 9, 0);
	trans[2][13]	= settr(172,0,31,27,27,"slide_pos_changed[num]?1", 1, 510, 0);
	...
	trans[2][18]	= settr(177,0,27,28,0,"((doors_status[num].lower==2))", 1, 2, 0);
	T = trans[2][27] = settr(186,0,0,0,0,"IF", 0, 2, 0);
	T = T->nxt	= settr(186,0,19,0,0,"IF", 0, 2, 0);
	    T->nxt	= settr(186,0,24,0,0,"IF", 0, 2, 0);
	trans[2][19]	= settr(178,0,20,29,0,"(((lock_water_level[num]!=2)&&(slide_status[num].lower==2)))", 1, 2, 0);
	trans[2][20]	= settr(179,0,21,30,30,"change_slide_pos[num]!2", 1, 9, 0);
	trans[2][21]	= settr(180,0,22,31,31,"slide_pos_changed[num]?1", 1, 510, 0);
	trans[2][22]	= settr(181,0,23,32,32,"change_doors_pos[num]!2", 1, 7, 0);
	trans[2][23]	= settr(182,0,33,33,33,"doors_pos_changed[num]?1", 1, 508, 0);
	trans[2][28]	= settr(187,0,33,1,0,".(goto)", 0, 2, 0);
	trans[2][24]	= settr(183,0,25,34,0,"((lock_water_level[num]==2))", 1, 2, 0);
	trans[2][25]	= settr(184,0,26,35,35,"change_doors_pos[num]!2", 1, 7, 0);
	trans[2][26]	= settr(185,0,33,36,36,"doors_pos_changed[num]?1", 1, 508, 0);
	trans[2][32]	= settr(191,0,33,1,0,".(goto)", 0, 2, 0);
	trans[2][29]	= settr(188,0,30,37,0,"((doors_status[num].lower==1))", 1, 2, 0);
	trans[2][30]	= settr(189,0,33,1,0,"(1)", 0, 2, 0);
	trans[2][33]	= settr(192,0,65,38,38,"observed_low[num]?1", 1, 505, 0);
	trans[2][34]	= settr(193,0,40,39,39,"request_high[num]?1", 1, 504, 0);
	T = trans[2][40] = settr(199,0,0,0,0,"IF", 0, 2, 0);
	T = T->nxt	= settr(199,0,35,0,0,"IF", 0, 2, 0);
	    T->nxt	= settr(199,0,38,0,0,"IF", 0, 2, 0);
	trans[2][35]	= settr(194,0,36,40,0,"((doors_status[num].lower==1))", 1, 2, 0);
	trans[2][36]	= settr(195,0,37,41,41,"change_doors_pos[num]!2", 1, 7, 0);
	trans[2][37]	= settr(196,0,47,42,42,"doors_pos_changed[num]?1", 1, 508, 0);
	...
	trans[2][42]	= settr(201,0,43,43,0,"((slide_status[num].lower==1))", 1, 2, 0);
	trans[2][43]	= settr(202,0,44,44,44,"change_slide_pos[num]!2", 1, 9, 0);
	trans[2][44]	= settr(203,0,62,45,45,"slide_pos_changed[num]?1", 1, 510, 0);
	...
	trans[2][49]	= settr(208,0,58,46,0,"((doors_status[num].higher==2))", 1, 2, 0);
	T = trans[2][58] = settr(217,0,0,0,0,"IF", 0, 2, 0);
	T = T->nxt	= settr(217,0,50,0,0,"IF", 0, 2, 0);
	    T->nxt	= settr(217,0,55,0,0,"IF", 0, 2, 0);
	trans[2][50]	= settr(209,0,51,47,0,"(((lock_water_level[num]!=1)&&(slide_status[num].higher==2)))", 1, 2, 0);
	trans[2][51]	= settr(210,0,52,48,48,"change_slide_pos[num]!1", 1, 9, 0);
	trans[2][52]	= settr(211,0,53,49,49,"slide_pos_changed[num]?1", 1, 510, 0);
	trans[2][53]	= settr(212,0,54,50,50,"change_doors_pos[num]!1", 1, 7, 0);
	trans[2][54]	= settr(213,0,64,51,51,"doors_pos_changed[num]?1", 1, 508, 0);
	trans[2][59]	= settr(218,0,64,1,0,".(goto)", 0, 2, 0);
	trans[2][55]	= settr(214,0,56,52,0,"((lock_water_level[num]==1))", 1, 2, 0);
	trans[2][56]	= settr(215,0,57,53,53,"change_doors_pos[num]!1", 1, 7, 0);
	trans[2][57]	= settr(216,0,64,54,54,"doors_pos_changed[num]?1", 1, 508, 0);
	trans[2][63]	= settr(222,0,64,1,0,".(goto)", 0, 2, 0);
	trans[2][60]	= settr(219,0,61,55,0,"((doors_status[num].higher==1))", 1, 2, 0);
	trans[2][61]	= settr(220,0,64,1,0,"(1)", 0, 2, 0);
	trans[2][64]	= settr(223,0,65,56,56,"observed_high[num]?1", 1, 506, 0);
	trans[2][67]	= settr(226,0,68,1,0,"break", 0, 2, 0);
	trans[2][68]	= settr(227,0,71,57,57,"num = (num+1)", 0, 2, 0);
	trans[2][69]	= settr(228,0,71,58,58,"((num==1))", 0, 2, 0); /* m: 70 -> 71,0 */
	reached2[70] = 1;
	trans[2][70]	= settr(0,0,0,0,0,"num = 0",0,0,0);
	trans[2][73]	= settr(232,0,74,1,0,"break", 0, 2, 0);
	trans[2][74]	= settr(233,0,0,59,59,"-end-", 0, 3500, 0);
	...
	trans[1][51]	= settr(100,0,72,78,0,"(((ship_status[shipid]==3)&&(ship_pos[shipid]!=1)))", 1, 2, 0);
	...
	trans[1][79]	= settr(128,2,84,91,91,"(((nr_of_ships_at_pos[(ship_pos[shipid]+1)]<2)||((ship_pos[shipid]+1)==1)))", 1, 6, 0); /* m: 80 -> 84,0 */
	reached1[80] = 1;
	...
	trans[1][86]	= settr(135,2,87,93,0,"(((nr_of_ships_at_pos[(ship_pos[shipid]+1)]==2)&&((ship_pos[shipid]+1)!=1)))", 1, 6, 0);
	...
	trans[1][91]	= settr(140,4,107,95,95,"(((doors_status[ship_pos[shipid]].higher==1)&&((nr_of_ships_at_pos[(ship_pos[shipid]+1)]<2)||((ship_pos[shipid]+1)==1))))", 1, 2, 0); /* m: 92 -> 107,0 */
	reached1[92] = 1;
	...
	trans[1][104]	= settr(153,0,105,99,0,"(((ship_status[shipid]==3)&&(ship_pos[shipid]==1)))", 1, 2, 0);
